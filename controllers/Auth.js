...
//   try {
//     const { firstName, lastName, email, password, phone, country, otp } =
//       req.body;
...
//       });
//     }
//     const existingStudent = await Student.findOne({ email }); //check Student already exist or not
//     if (existingStudent) {
...
//     const response = await OTP.find({ email }).sort({ createdAt: -1 }).limit(1); //find most recent OTP stored for the Student or most recent OTP generated for Student;
...
//       // if otp entered by Student != actual otp then PRINT Invalid OTP;
//       return res.status(400).json({
//         // here otp is entered by Student and response[0].otp is generated by controller;
//         success: false,
//         message: 'Invalid OTP',
//       });
//     }
...
//       contactNumer: null,
//     });
//     //created entry in Student in DB
...
//       success: false,
//       message: 'Student cannot be registrered. Please try again',
//     });
//   }
// };
const signUp = async (req, res) => {
  try {
    ...
    const student = await Student.create({
      ...
      phone,
      country,
      referral: referral || null, // Store referral if provided, otherwise null
      approved: true,
      additionalDetails: profileDetails._id,
      image: `https://api.dicebear.com/5.x/initials/svg?seed=${firstName} ${lastName}`,
    });
